...
int caloriesPerDate(DateTime date, List<Meal> meals) {
  Iterable<Meal> filteredCalories = meals.where(
    (meal) =>
        meal.consumedOn.day == date.day &&
        meal.consumedOn.month == date.month &&
        meal.consumedOn.year == date.year,
  );
  return filteredCalories.fold<int>(
    0,
    (prev, next) => prev + next.caloriesQuantity,
  );
}
